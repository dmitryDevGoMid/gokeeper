...
var serverFilesList *httptest.Server

// Создаем тестовый сервер
func SetServerFilesList(t *testing.T) {

	// Create a test server
	serverFilesList = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			t.Errorf("Expected POST request, got %s", r.Method)
			return
		}

		if r.URL.Path != "/list/files" {
		...
		if payload.Type != "get_list_files" {
		...
		data1, err := encrypt.EncryptByClientKeyParts(string(`{"_id": "1","filename": "fotocate.jpg", "length": 16808746,"chunk_size": 50000, "upload_date": "2024-09-21T17:25:36.433Z"}`), PublicClientKey)
		if err != nil {
			log.Println("asimencrypt failed to encrypt", err)
		}

		//Берем ключ клиента публичны и шифруем данные
		data2, err := encrypt.EncryptByClientKeyParts(string(`{"_id": "2","filename": "fotodog.jpg", "length": 16808746,"chunk_size": 50000,"upload_date": "2024-09-21T17:25:36.433Z"}`), PublicClientKey)
		if err != nil {
			log.Println("asimencrypt failed to encrypt", err)
		}
...
func TestFilesList(t *testing.T) {
	...
	SetKey(encrypt)
	SetServerFilesList(t)

	defer serverFilesList.Close()
	...
	fileslist := data.RequestHTTP["fileslist"]

	fileslist.URL = serverFilesList.URL + "/list/files"
	data.User.Token = "test-token"
	data.RequestHTTP["fileslist"] = fileslist
	...
	pathFile := "testdata/filelist/menu.out"
	...
	pathFile = "testdata/filelist/select.id.2.out"
	if fileExists(pathFile) {
		dataCompare, err = readFromFile(pathFile)
		if err != nil {
			fmt.Println(err)
