...
func (m Login) SetUsername(username string) {
	m.inputs[0].SetValue(username)
}

func (m Login) SetPassword(password string) {
	m.inputs[1].SetValue(password)
}

func NewLogin(model *model.Data) Page {
	login := initialModel()
	login.data = model
	return login
}

func (m Login) NextStepBack() {
	m.data.NextStep.NextStepByName = "topmenu"
}

func (m Login) NextStepToSendAuthDataToServer() {
	m.data.NextStep.NextStepByName = "requesthttp"
	m.data.NextStep.RequestByName = "login"
}

func (m Login) SaveAuthorizedToModel() {
...
func (m Login) View() string {
	...
	b.WriteString("Form Login to Application\n\n")
	for i := range m.inputs {
